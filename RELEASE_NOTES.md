...
- **Standard test tooling.** All tests migrated from a custom console runner to MSTest with identical inputs and expectations; `dotnet test` (with TRX result files) is now the single entry point. The suite grew from 82 to 97 tests. The 10MB offset-lookup gate now measures per-lookup p95 against its 200ms requirement instead of incorrectly applying one 200ms budget to 60 operations.
